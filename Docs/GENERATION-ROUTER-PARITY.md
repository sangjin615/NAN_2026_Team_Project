...
| 계약 검증 실패 | 0/8 | **0/8** |
| 지연 중앙 | 10.9초 | **9.4초** |
| 지연 범위 | 9.5~20.1초 | **8.3~11.9초** |
| 서로 다른 어미 | 5가지 × 6판, 4가지 × 1판 | 5가지 × 5판, 4가지 × 3판 |
| 서로 다른 첫머리 | 8/8 × 5판, 7/8 × 2판 | 8/8 × 3판, 7/8 × 5판 |
| 규칙 밖 어미 | 0건 | 0건 |

**꼬리가 짧아졌다.** 최대가 20.1 → 11.9초다. 입력이 절반 아래로 줄었으니
설명이 된다. 이 항목은 8판씩 양쪽 다 있어 비교적 믿을 만하다.

**LOT ID 어긋남은 한 번도 없었다.** 자리 고정을 잃은 대가를 걱정했는데
계약 검증 실패 0/8 이다.

**단정하면 안 되는 것 둘.**

- static 1/8 → 0/8 은 **사건 하나짜리다.** "고쳤다"고 읽으면 안 된다
- 어미 4가지 판이 1 → 3판, 첫머리 7/8 판이 2 → 5판으로 늘었다. 8판 표본으로는
  실제 저하인지 편차인지 가르지 못한다. 다음에 재는 사람이 이 방향을 다시
  보면 그때 판단할 근거가 생긴다

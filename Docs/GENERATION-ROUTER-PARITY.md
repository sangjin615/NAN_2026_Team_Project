...
## static 이 대놓고 템플릿이던 것을 고쳤다 (2026-08-08)

낙하율은 16판에 1판이다. 그런데 **걸린 사람에게는 그 화면이 이 게임이다.**
그리고 실패를 알려주는 표시가 없으니 "생성이 느렸다" 가 아니라 "이 정도밖에
못 만들었다" 로 읽힌다.

예전 `deterministicFallback` 은 카테고리당 문장이 하나였다.

```
청자 항아리의 1번째 표면의 유약 자국이 은은하게 남아 있다.
백자 대접의 7번째 표면의 유약 자국이 은은하게 남아 있다.
```

하루가 8 LOT 인데 카테고리는 6종이라 **비둘기집 원리로 매일 최소 두 쌍이 글자
그대로 같았다.** 어미도 여섯 틀 중 다섯이 `남아 있다` 라 사실상 2가지였다.

### 지난 생성물을 은행으로 만들었다

새로 지어내지 않았다. `reports/live-generation` 에 쌓인 **실제 생성물**에서
걷었다 — 이미 비용을 치렀고 계약을 통과한 문장들이다.
`npm run build:fallback-bank` 로 다시 만들 수 있고 결과는
`data/fallback-copy.json` 이다.

거르는 단계가 넷이다.

1. **실제 `qualityErrors` 로 검증한다.** 후보를 여덟씩 묶어 검증기에 먹이고
   걸린 자리만 버린다. 카테고리 어휘·어미·한 문장 규칙을 여기서 다시 적으면
   갈라지므로 복제하지 않는다 (`AGENTS.md`)
2. 어미만 다른 중복은 줄기 기준으로 하나만 남긴다
3. 카탈로그 60종 이름이 든 문장은 버린다
4. **품목 머리 명사 59종이 든 문장은 예비로 돌린다** — "소금단지의 뚜껑에" 가
   청자 항아리에 붙는 것을 막는다. 전부 버리지 않는 것은 `시계`·`장식` 처럼
   머리 명사이면서 카테고리 필수 어휘인 것이 있어서다

### 얇은 카테고리는 따로 뽑아 채운다

실제 판은 카테고리가 골고루 섞여 나오므로 적게 등장한 카테고리는 표본이
모자란다. **PNT 가 깨끗한 문장 6개뿐이었다.**

`tools/collect-fallback-samples.mjs` 가 **그 카테고리만 8개인 하루**를 만들어
배포본에 보낸다. 한 번에 최대 8문장이 나오고, 결과를 기존 기록과 같은 모양으로
저장하므로 은행 만들기가 그대로 읽는다.

```bash
node tools/collect-fallback-samples.mjs PNT 8   # 실제 공급자를 부른다 — 돈이 나간다
npm run build:fallback-bank
```
...
**JEW 만 유독 힘들었다.** 8판에서 3판, 10판에서 4판만 성공했다. 사유는
`description has unsafe ending` 이다 — 모델이 허용된 다섯 어미 밖으로 나갔고
복구도 실패한 뒤 `gpt-4o-mini` 가 타임아웃됐다.

**다만 이것을 실제 낙하율로 읽으면 안 된다.** 이 도구는 그 카테고리만 8개인
하루를 만든다. 실제 판은 카테고리가 섞여 나오므로 훨씬 쉽다 — 같은 시기
섞인 8판 측정에서는 16판에 1판만 떨어졌다. **장신구 여덟을 한 번에 쓰라고
하면 모델이 어미를 놓친다**는 것이 여기서 드러난 사실이고, 실제 플레이에서
JEW 가 한 판에 최대 5개인 것을 생각하면 지금은 경계 밖이다.

**BOK 16개는 아직 다른 것들보다 얇다.** 하루 최대 5개는 여유 있게 넘지만
같은 방법으로 채울 수 있다.

### 지어낸 사물 명사도 걸러낸다

머리 명사 목록으로는 안 걸리는 것이 있었다. 배포 후 실측에서 청자 항아리에
**"제비문이 찻잔의 손잡이에 새겨져 있고"** 가 붙었다. `찻잔` 은 카탈로그에 없는,
모델이 지어낸 사물이다.

`strayObjectNouns` 목록을 만들어 조사가 붙은 형태로만 거른다 — 그냥 포함으로
보면 `잔금`·`함께` 같은 말이 걸린다. CER 에서 6건이 빠졌고(50 → 44) 다른
카테고리는 손실이 없었다. **이 목록은 지금까지 본 사례의 기록이다.** 새로
발견되면 더한다.

같은 실측에서 황동 나침반에 "**은제** 표면에" 가 붙은 것도 봤다. 이건 못
고친다 — MET 은행에 황동과 은제가 함께 들어 있고, 카테고리 단위로 돌려쓰는 한
재질 어긋남은 남는다.

**은행의 원재료는 git 에 없다.** `reports/live-generation` 은 미추적이고
`AGENTS.md` 가 사용자 파일로 두라고 한 자리다. 그래서 `data/fallback-copy.json`
(결과물)만 커밋한다. 새로 clone 한 곳에서는 은행을 다시 만들 수 없고, 채우려면
위 명령으로 표본부터 모아야 한다.
...
| | 예전 템플릿 | **은행** |
|---|---|---|
| 하루 안 중복 설명 | **매일 최소 2쌍** (구조적) | **0건** |
| 하루 평균 서로 다른 어미 | 2가지 | **3.99가지** (최소 2) |
| 하루 평균 서로 다른 첫머리 | — | **7.27 / 8** |
| 계약 검증 실패 | 0 | **0 / 480일** |
| 다른 품목 이름 새어듦 | — | **0건 / 3,840 LOT** |
| 지어낸 사물 명사 | — | **0건 / 3,840 LOT** |

참고로 실제 생성의 어미는 8판 평균 4.6가지였다. **fallback 이 생성에 근접한다.**

### 남은 한계 — 정직하게 적는다

**카테고리 단위로 돌려쓴다.** 도자기 설명이 다른 도자기에 붙는다. 카탈로그
이름은 걸렀지만 그 밖의 사물 명사는 남는다. 배포 후 실측에서 실제로 나왔다 —
청자 항아리에 "제비문이 **찻잔**의 손잡이에 새겨져 있고" 가 붙었다. `찻잔` 은
카탈로그 품목명이 아니라 필터가 잡지 못했다.

**그래도 바꾼 쪽이 낫다.** 예전 것은 100% 티가 났고 이것은 가끔 어긋난다.
완전히 없애려면 품목별로 은행을 나눠야 하는데 그러면 문장 수가 모자란다.

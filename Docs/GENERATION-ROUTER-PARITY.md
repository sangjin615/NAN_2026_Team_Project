...
**한 번에 만들고, 실패한 LOT 만 골라 다시 만든다**(`dailyRepairIndices`).
라우터가 그것을 안 하는 이유는 공급자 타임아웃 7~9초 안에 하루치가 안 들어온다는
판단이었는데, 배포본은 7.0~21.9초에 해낸다.

**타임아웃이 진짜 제약인지 다시 재볼 값어치가 있다.** 지금 라우터로 옮긴다면
순서는 이렇다.

1. 한 번에 만들기로 되돌리고 타임아웃을 올린다
2. 실패하면 `dailyRepairIndices` 로 그 LOT 만 복구한다
3. 그래도 안 되면 다음 공급자

측정 없이 옮기지 말 것. 위 표가 그 이유다.

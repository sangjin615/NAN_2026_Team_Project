...
- 검증을 통과한 `displayName`, `description`은 카탈로그에 표시한다.
- `rumor`, `setHint`, `npcReaction`은 실제 경매의 진행 로그에 표시한다.
- 생성 문자열은 HTML 이스케이프 후 렌더링한다.
- 공급자 장애나 두 번의 검증 실패가 발생하면 결정적 fallback이 다섯 표시 필드를 모두 제공한다.
- 기준가, 실제 가치, 등급, 입찰가와 낙찰 결과는 계속 엔진만 결정한다.

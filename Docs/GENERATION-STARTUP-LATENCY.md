...
키가 있는 쪽에서 한 판 돌려 볼 것.

- 슬롯 화면에 들어가자마자 서버 로그에 `run-blueprint` 가 찍히는가
- 새 여정 시작을 눌렀을 때 blueprint 를 **다시 부르지 않는가**
- 로딩이 2초 바닥값 근처로 끝나는가 (미리 만들기가 끝났다면)
- 날짜 전환이 즉시 되는가, 그리고 그 직후 `day N+2` 가 뒤에서 나가는가
- 슬롯 화면을 몇 번 들락거려도 `run-blueprint` 가 한 번만 나가는가
- 타이틀로 나갔다 다시 들어오면 **새 시드로** 한 번 더 나가는가

브라우저 자동화로는 확인하지 못했다. 이 세션에서는 미리보기 패널이 280px 폭으로
검게만 렌더되어 클릭도 스크린샷도 되지 않았다.

...
const runRequest={schemaVersion:'1.0',mode:'run-blueprint',runSeed:seed,days:12,categories:['CER','CLK','PNT','BOK','MET','JEW'],sets:sets.map((set)=>({setId:set.setId,themeKey:set.themeKey,lotIds:set.lotIds})),bots:[{botId:'nemesis',role:'숙적'},{botId:'drifter-a',role:'유랑 상인'},{botId:'drifter-b',role:'유랑 상인'}],relics:balance.relics.list.map(({id,tier})=>({relicId:id,tier})),market};
